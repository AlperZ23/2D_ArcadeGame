# ALTERNATIVE SHOOTING VERSION(I DONT LIKE IT)
import pygame
import random
import math
from os import path
...
mob_img = pygame.image.load(path.join(img_dir, 'mob2.png'))
background_rect = []
farm_img = pygame.image.load(path.join(img_dir, 'base1.png'))
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            ...
            if not polska:
                ...
                bullet.way = random.randrange(4)
                #bullet.image.fill(colors[random.randrange(0, 6)])
...
while running:
    ...
    for hit in hits:
        score += 1
        enemies -= 1
        if  random.random() < 0.9 and not polska and pygame.time.get_ticks() - last_polska > 1200 :
            gr = Grass()
            grasses.add(gr)
            all_sprites.add(grasses)

    hits = pygame.sprite.spritecollide(player, mobs, True ,pygame.sprite.collide_circle) # change on False to get pizdec mode
    for hit in hits:
        player.shield -= 10
        enemies -= 1
        if player.shield <= 0:
    ...
    hits = pygame.sprite.spritecollide(farm, mobs, True, pygame.sprite.collide_circle)
    for hit in hits:
        farm.shield -= 10
        enemies -= 1
        if farm.shield <= 0:
    ...
    hits = pygame.sprite.spritecollide(player, grasses, True, pygame.sprite.collide_circle)  # the beggining of POLSKA mode
    for hit in hits:
        polska = True
        now = pygame.time.get_ticks()
    ...
    if polska:
        ...
        draw_text(screen, 'polska ', 18, 70.2 + random.randrange(-2,2), 60, RED)
        if polska_timer > 20:
         draw_text(screen, str(polska_timer), 18, 101.2 , 60,GREEN)
        elif polska_timer > 10:
            draw_text(screen, str(polska_timer), 18, 101.2 + random.randrange(-1,1), 60, YELLOW)
        else:
            draw_text(screen, str(polska_timer), 18, 101.2 + random.randrange(-2,2), 60, RED)

    # hp bars
    if not polska:
        draw_shield_bar(screen, 5, 5, player.shield, RED, 1)
    else:
        draw_shield_bar(screen, 5, 5, player.shield, colors[random.randrange(0, 6)], 1)
